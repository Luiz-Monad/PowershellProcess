...
namespace Nito.AsyncEx.Synchronous
{
    ...
    public static class TaskExtensions
    {
        ...
        public static void WaitAndUnwrapException(this Task task, CancellationToken? cancellationToken = null)
        {
            if (task == null)
            ...
            try
            {
                ...
                {
                    var ct = cancellationToken ?? throw new NullReferenceException();
                    task.Wait(ct);
                }
            }
            catch (AggregateException ex)
            {
                throw ExceptionHelpers.PrepareForRethrow(ex.InnerException ?? ex);
            }
        }
        ...
        /// <exception cref="OperationCanceledException">The <paramref name="cancellationToken"/> was cancelled before the <paramref name="task"/> completed, or the <paramref name="task"/> raised an <see cref="OperationCanceledException"/>.</exception>
        public static TResult WaitAndUnwrapException<TResult>(this Task<TResult> task, CancellationToken? cancellationToken = null)
        {
            if (task == null)
                throw new ArgumentNullException(nameof(task));
            try
        ...
        public static void WaitWithoutException(this Task task, CancellationToken? cancellationToken = null)
        {
            if (task == null)
                throw new ArgumentNullException(nameof(task));
            try
